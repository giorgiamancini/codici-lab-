...
/*******************************************************************************
* Function Name: Timer_LED_Clock_Start
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Timer_LED_Clock_Start(void) 
{
    /* Set the bit to enable the clock. */
    Timer_LED_Clock_CLKEN |= Timer_LED_Clock_CLKEN_MASK;
	Timer_LED_Clock_CLKSTBY |= Timer_LED_Clock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Timer_LED_Clock_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Timer_LED_Clock_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    Timer_LED_Clock_CLKEN &= (uint8)(~Timer_LED_Clock_CLKEN_MASK);
	Timer_LED_Clock_CLKSTBY &= (uint8)(~Timer_LED_Clock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Timer_LED_Clock_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Timer_LED_Clock_StopBlock(void) 
{
    if ((Timer_LED_Clock_CLKEN & Timer_LED_Clock_CLKEN_MASK) != 0u)
    {
...
#if defined(Timer_LED_Clock__CFG3)
        CLK_DIST_AMASK = Timer_LED_Clock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Timer_LED_Clock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Timer_LED_Clock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Timer_LED_Clock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Timer_LED_Clock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Timer_LED_Clock_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Timer_LED_Clock_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        Timer_LED_Clock_CLKSTBY &= (uint8)(~Timer_LED_Clock_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: Timer_LED_Clock_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Timer_LED_Clock_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = Timer_LED_Clock_GetSourceRegister();
    uint16 oldDivider = Timer_LED_Clock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Timer_LED_Clock_CLKEN & Timer_LED_Clock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Timer_LED_Clock_DIV_PTR, clkDivider);
                Timer_LED_Clock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Timer_LED_Clock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Timer_LED_Clock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Timer_LED_Clock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Timer_LED_Clock_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 Timer_LED_Clock_GetDividerRegister(void) 
{
    return CY_GET_REG16(Timer_LED_Clock_DIV_PTR);
}


/*******************************************************************************
* Function Name: Timer_LED_Clock_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Timer_LED_Clock_SetModeRegister(uint8 modeBitMask) 
{
    Timer_LED_Clock_MOD_SRC |= modeBitMask & (uint8)Timer_LED_Clock_MODE_MASK;
}


/*******************************************************************************
* Function Name: Timer_LED_Clock_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Timer_LED_Clock_ClearModeRegister(uint8 modeBitMask) 
{
    Timer_LED_Clock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Timer_LED_Clock_MODE_MASK));
}


/*******************************************************************************
* Function Name: Timer_LED_Clock_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Timer_LED_Clock_GetModeRegister(void) 
{
    return Timer_LED_Clock_MOD_SRC & (uint8)(Timer_LED_Clock_MODE_MASK);
}


/*******************************************************************************
* Function Name: Timer_LED_Clock_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Timer_LED_Clock_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = Timer_LED_Clock_GetDividerRegister();
    uint8 oldSrc = Timer_LED_Clock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Timer_LED_Clock_MOD_SRC |= CYCLK_SSS;
        Timer_LED_Clock_MOD_SRC =
            (Timer_LED_Clock_MOD_SRC & (uint8)(~Timer_LED_Clock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Timer_LED_Clock_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Timer_LED_Clock_GetSourceRegister(void) 
{
    return Timer_LED_Clock_MOD_SRC & Timer_LED_Clock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Timer_LED_Clock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Timer_LED_Clock_SetPhaseRegister(uint8 clkPhase) 
{
    Timer_LED_Clock_PHASE = clkPhase & Timer_LED_Clock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Timer_LED_Clock_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Timer_LED_Clock_GetPhaseRegister(void) 
{
    return Timer_LED_Clock_PHASE & Timer_LED_Clock_PHASE_MASK;
}
